...
export function validateEnv(config: Record<string, unknown>): Record<string, unknown> {
  ...
  if (config.NODE_ENV === 'production') {
    ...
    const insecure = ['default_jwt_secret_change_in_production', 'default_refresh_secret_change_in_production'];
    if (insecure.includes(config.JWT_SECRET as string) || insecure.includes(config.JWT_REFRESH_SECRET as string)) {
